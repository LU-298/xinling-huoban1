...
function speakText(text) {
    if ('speechSynthesis' in window) {
        ...
        u.onend = () => { if (robot) robot.classList.remove('speaking'); };
    }
}
</script>
""", height=0)
...
.avatar-container { width: 160px; height: 160px; margin: 0 auto; border-radius: 50%; overflow: hidden; border: 5px solid white; box-shadow: 0 10px 40px rgba(0,0,0,0.3); position: relative; }
.avatar-img { width: 100%; height: 100%; object-fit: cover; animation: idle 4s ease-in-out infinite; }
@keyframes idle { 0%,100% { transform: scale(1) translateY(0); } 50% { transform: scale(1.03) translateY(-3px); } }
/* 眨眼效果 - 眼睛遮罩 */
.avatar-container::after { content: ''; position: absolute; top: 25%; left: 10%; width: 80%; height: 25%; background: #2d5a2d; opacity: 0; transition: opacity 0.1s; border-radius: 50%; }
.avatar-container.blink::after { opacity: 1; animation: blink-anim 0.2s ease-in-out; }
@keyframes blink-anim { 0%,100% { opacity: 0; } 50% { opacity: 1; } }
/* 说话效果 */
.avatar-container.speaking .avatar-img { animation: speak 0.4s ease-in-out infinite; }
@keyframes speak { 0%,100% { transform: scale(1) translateY(0); } 25% { transform: scale(1.02) translateY(-2px) rotate(-1deg); } 75% { transform: scale(1.02) translateY(-2px) rotate(1deg); } }
.robot-name { color: white; font-size: 24px; font-weight: bold; margin-top: 15px; text-shadow: 0 2px 10px rgba(0,0,0,0.2); }
...
.msg-content { display: inline-block; max-width: 80%; padding: 12px 16px; border-radius: 18px; font-size: 14px; line-height: 1.5; word-break: break-word; }
...
.input-section { padding: 15px 20px; background: white; border-top: 1px solid #eee; display: flex; gap: 10px; align-items: center; }
.input-box { flex: 1; border: none !important; border-radius: 25px !important; padding: 14px 20px !important; background: #f5f5f5 !important; font-size: 14px !important; box-shadow: none !important; }
.input-box:focus { outline: none !important; }
...
.hint { text-align: center; color: #999; font-size: 12px; padding: 10px; }
</style>
...
# 眨眼JS
st.components.v1.html("<script>setInterval(()=>{const a=window.parent.document.querySelector('.avatar-container');if(a&&!a.classList.contains('speaking')){a.classList.add('blink');setTimeout(()=>a.classList.remove('blink'),250);}},4000);</script>", height=0)

# 顶部
e = EMOTIONS[st.session_state.cur_emotion]
speak_class = "speaking" if st.session_state.get("speaking") else ""
st.markdown(f"""<div class="top-section"><div class="avatar-container {speak_class}"><img src="{ROBOT_URL}" class="avatar-img" /></div><div class="robot-name">🌱 心灵伙伴</div><div class="emotion-badge">{e["emoji"]} {e["name"]}</div></div>""", unsafe_allow_html=True)
...
st.markdown('</div></div><div class="hint">💡 按 Enter 发送消息</div>', unsafe_allow_html=True)
